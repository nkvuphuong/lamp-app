...
if (rabbit_queue::needConsumers() > 0) {
    ...
    try {
        ...
        $callback = static function (PhpAmqpLib\Message\AMQPMessage $msg) use (&$creatingOrderMessages, $redisQueueClient, $rabbitmqConnection) {
            $redisQueueSize = 10;
            ...
            if (count($creatingOrderMessages) >= $redisQueueSize) {
                ...
                if (rabbit_queue::isDestroyed($rabbitmqConnection->getConsumerId())) {
                    echo "[*] This consumer {$rabbitmqConnection->getConsumerId()} is destroyed \n";
